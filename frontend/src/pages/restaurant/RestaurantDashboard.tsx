...
export default function RestaurantDashboard() {
  ...
  const maxSales = Math.max(1, ...days.map((d: any) => Number(d.sales) || 0))
      ...
      >
        <div className="flex items-center justify-between px-4 py-4 border-b border-semay-100">
          ...
          <button
            type="button"
            onClick={closeMobileMenu}
            className="w-9 h-9 rounded-xl flex items-center justify-center text-semay-500 hover:bg-semay-100"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-4 border-t border-semay-100">
          <button
            type="button"
            onClick={handleLogout}
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-semay-500 hover:bg-semay-100"
          >
            <LogOut className="w-4 h-4" />
            Logout
          </button>
        ...
        <header className="bg-white border-b border-semay-200 px-4 md:px-6 h-14 flex items-center justify-between sticky top-0 z-30">
          <div className="flex items-center gap-3 min-w-0">
            <button
              ...
              className="md:hidden w-9 h-9 shrink-0 rounded-xl flex items-center justify-center text-semay-700 hover:bg-semay-100"
            >
          ...
          {loading ? (
            ...
            <>
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                {[
                  { label: "TODAY SALES", value: `${data?.today?.sales ?? 0} ETB` },
                  { label: "TODAY ORDERS", value: data?.today?.orders ?? 0 },
                  { label: "ACTIVE NOW", value: data?.today?.activeOrders ?? 0 },
                  {
                    label: "HEALTH SCORE",
                    value: health,
                    sub: health >= 80 ? "Strong" : health >= 50 ? "OK" : "Needs attention",
                  },
                ].map((k) => (
                  <div
                    key={k.label}
                  ...
                  >
                    ...
                    {k.sub && <div className="text-xs text-semay-500 mt-0.5">{k.sub}</div>}
                  </div>
                ))}
              </div>
              ...
              <div className="bg-white border border-semay-100 rounded-xl p-4 shadow-sm">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Link to="/kds" className="text-xs font-medium text-semay-600 hover:text-semay-900">
                    Open Kitchen →
                  </Link>
                </div>
              ...
              <div className="bg-white border border-semay-100 rounded-xl p-4 md:p-5 shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
                  ...
                  <div className="text-xs text-semay-500">
                    Week: {data?.week?.sales ?? 0} ETB · avg {data?.week?.avgDaily ?? 0}/day
                  </div>
                </div>
                <div className="flex items-end gap-2 h-36">
                  {days.length === 0 ? (
                    <p className="text-sm text-semay-400 w-full text-center py-8">No week data</p>
                  ) : (
                    days.map((d: any) => {
                      const sales = Number(d.sales) || 0
                      const h = sales <= 0 ? 4 : Math.max(8, Math.round((sales / maxSales) * 100))
                      return (
                        <div
                          key={d.date}
                          className="flex-1 flex flex-col items-center gap-1 group relative h-full justify-end"
                        >
                          <div className="absolute bottom-full mb-1 opacity-0 group-hover:opacity-100 text-[10px] bg-semay-900 text-white px-1.5 py-0.5 rounded whitespace-nowrap z-10">
                            {sales} ETB · {d.orders || 0} orders
                          </div>
                          <div
                            className="w-full rounded-t-md bg-semay-800/90 hover:bg-semay-900 transition-all min-h-[4px]"
                            style={{ height: `${h}%` }}
                          />
                      ...
                      <ul className="text-sm space-y-1">
                        {data.lowStock.map((m: any) => (
                          <li key={m.id} className="flex justify-between">
                            <span>{m.name}</span>
                            <span className="text-amber-600">{m.stockQty}</span>
                          </li>
                        ))}
                      </ul>
                    )}
