...
import { Minus, Plus, Send, UtensilsCrossed } from "lucide-react"
...
export default function CustomerOrder() {
  ...
  const send = async () => {
    ...
    try {
      ...
      setCart([])
      setMsg("Order sent to the kitchen. Thank you!")
    } catch (e: any) {
  ...
  return (
    <div className="min-h-svh bg-[#f6f4f0] pb-32">
      ...
      <header className="relative overflow-hidden bg-stone-900 text-white">
        ...
        <div className="relative px-5 pt-10 pb-8 max-w-lg mx-auto">
          <div className="flex items-end gap-4">
            <div className="w-16 h-16 rounded-2xl bg-white/10 border border-white/20 overflow-hidden shrink-0 backdrop-blur-sm flex items-center justify-center">
              {org?.photoUrl ? (
                <img src={org.photoUrl} alt="" className="w-full h-full object-cover" />
              ) : (
            ...
            <div className="min-w-0 pb-0.5">
              ...
              <p className="text-sm text-white/70 mt-0.5">Order from your table</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-lg mx-auto px-4 -mt-3 relative z-10">
        <div className="bg-white rounded-2xl border border-stone-200/80 shadow-sm p-3 flex items-center gap-3">
          <label className="text-xs text-stone-500 shrink-0 pl-1">Table</label>
          <input
            value={table}
            onChange={(e) => setTable(e.target.value)}
            className="flex-1 px-3 py-2 rounded-xl bg-stone-50 border border-stone-100 text-sm font-medium"
            placeholder="e.g. 5"
          />
        </div>
      </div>
        ...
        <div className="max-w-lg mx-auto px-4 mt-3">
          <p
            className={`text-sm px-4 py-3 rounded-2xl ${
              msg.includes("Thank") || msg.includes("sent")
        ...
        {loading ? (
          ...
          filtered.map((m) => (
            <button
              ...
              type="button"
              onClick={() => add(m)}
              className="w-full flex gap-3 bg-white border border-stone-200/90 rounded-2xl p-2.5 text-left shadow-sm hover:shadow-md hover:border-stone-300 active:scale-[0.99] transition"
            >
              ...
              <div className="flex-1 min-w-0 py-1 pr-1 flex flex-col justify-between">
                <div>
                  <div className="font-semibold text-stone-900 text-[15px] leading-snug">
                    {m.name}
                  </div>
                  {m.category ? (
                    <div className="text-[11px] text-stone-400 mt-0.5">{m.category}</div>
                  ) : null}
                  {m.description ? (
                ...
                <div className="flex items-center justify-between mt-2">
                  <span className="text-sm font-semibold tabular-nums text-stone-900">
                    ...
                    <span className="text-xs font-medium text-stone-400">ETB</span>
                  </span>
        ...
        <div className="fixed bottom-0 inset-x-0 z-20">
          <div className="max-w-lg mx-auto bg-white/95 backdrop-blur border-t border-stone-200 shadow-[0_-8px_30px_rgba(0,0,0,0.08)] px-4 pt-3 pb-5 space-y-2">
              ...
              {cart.map((c) => (
                <div key={c.name} className="flex items-center justify-between gap-2 text-sm">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    <span className="w-5 text-center tabular-nums font-medium">{c.qty}</span>
                    <button
                      type="button"
                      onClick={() => setQty(c.name, 1)}
                      className="w-7 h-7 rounded-full border border-stone-200 flex items-center justify-center"
            ...
            <button
              ...
              disabled={sending}
              onClick={send}
              className="w-full flex items-center justify-center gap-2 bg-stone-900 text-white py-3.5 rounded-2xl text-sm font-medium disabled:opacity-50"
            >
